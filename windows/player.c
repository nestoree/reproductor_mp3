...
typedef struct Player {
    ...
    gboolean tree_mode;
    GtkWidget *tree_toggle_btn;
    gchar *folder_current_path;
} Player;
...
static gboolean song_matches_search(Song *song, const gchar *search_text) {
    if (!search_text || search_text[0] == '\0') return TRUE;
    gchar *lower = g_utf8_strdown(search_text, -1);
    gboolean found = FALSE;

    if (song->title) {
        gchar *s = g_utf8_strdown(song->title, -1);
        if (strstr(s, lower)) found = TRUE;
        g_free(s);
    }
    if (!found && song->artist) {
        gchar *s = g_utf8_strdown(song->artist, -1);
        if (strstr(s, lower)) found = TRUE;
        g_free(s);
    }
    if (!found && song->album) {
        gchar *s = g_utf8_strdown(song->album, -1);
        if (strstr(s, lower)) found = TRUE;
        g_free(s);
    }
    if (!found) {
        gchar *s = g_utf8_strdown(song->filepath, -1);
        if (strstr(s, lower)) found = TRUE;
        g_free(s);
    }
...
static gint song_sort_by_title(gconstpointer a, gconstpointer b) {
    Song *sa = (Song *)a;
    Song *sb = (Song *)b;
    gchar *taf = NULL, *tbf = NULL;
    const gchar *ta = sa->title && sa->title[0] ? sa->title : (taf = g_path_get_basename(sa->filepath));
    const gchar *tb = sb->title && sb->title[0] ? sb->title : (tbf = g_path_get_basename(sb->filepath));
    gint r = g_utf8_collate(ta, tb);
    if (r != 0) { g_free(taf); g_free(tbf); return r; }
    r = g_strcmp0(sa->filepath, sb->filepath);
    g_free(taf); g_free(tbf);
    return r;
}
...
static void player_update_playlist_row(Player *player, gint index) {
    GList *children = gtk_container_get_children(
        GTK_CONTAINER(player->playlist_box));
    for (GList *c = children; c; c = c->next) {
        gint val = GPOINTER_TO_INT(
            g_object_get_data(G_OBJECT(c->data), "song-idx"));
        if (val <= 0) continue;
        if (val - 1 == index) {
            Song *song = (Song *)g_list_nth_data(player->songs, index);
            if (!song) break;
            GtkWidget *label = gtk_bin_get_child(GTK_BIN(c->data));
            if (label) {
                gchar *display;
                if (song->metadata_loaded && strlen(song->artist) > 0) {
                    if (player->tree_mode)
                        display = g_strdup_printf("      %s  -  %s",
            ...
            gtk_list_box_select_row(GTK_LIST_BOX(player->playlist_box),
                                    GTK_LIST_BOX_ROW(c->data));
            break;
        }
    }
    ...
    case GST_MESSAGE_EOS: {
        ...
        if (player->songs && total > 0) {
            if (player->shuffle) {
                ...
                do {
                    next = g_random_int_range(0, total);
                } while (next == player->current_index);
                player->current_index = next;
                player_load_song(player, player->current_index);
                player_update_playlist_row(player, player->current_index);
            } else if (player->current_index < total - 1) {
...
static void player_next(Player *player) {
    ...
    if (player->shuffle) {
        ...
        do {
            next = g_random_int_range(0, len);
        } while (next == player->current_index);
    } else {
        next = player->current_index + 1;
        if (next >= len) next = 0;
    }
...
static void player_refresh_playlist(Player *player) {
    ...
    const gchar *search = player->search_entry
        ? gtk_entry_get_text(GTK_ENTRY(player->search_entry)) : "";

    if (player->tree_mode) {
        GList *dirs = NULL;
        GList *song_items = NULL;

        if (!player->folder_current_path) {
            GList *folder_pairs = NULL;
            for (GList *l = player->folders; l; l = l->next) {
                gchar *base = g_path_get_basename((gchar *)l->data);
                folder_pairs = g_list_append(folder_pairs, g_strdup(base));
            }
            folder_pairs = g_list_sort(folder_pairs, (GCompareFunc)g_utf8_collate);

            for (GList *l = folder_pairs; l; l = l->next) {
                gchar *base = (gchar *)l->data;
                gchar *fullpath = NULL;
                for (GList *fl = player->folders; fl; fl = fl->next) {
                    gchar *fb = g_path_get_basename((gchar *)fl->data);
                    if (g_strcmp0(fb, base) == 0) {
                        fullpath = g_strdup((gchar *)fl->data);
                        g_free(fb);
                        break;
                    }
                    g_free(fb);
                ...
                gchar *markup = g_strdup_printf("\U0001f4c1 %s", base);
                gtk_label_set_markup(GTK_LABEL(lab), markup);
                g_free(markup);
                gtk_label_set_xalign(GTK_LABEL(lab), 0.0);
                GtkWidget *row = gtk_list_box_row_new();
                g_object_set_data(G_OBJECT(row), "song-idx", GINT_TO_POINTER(-2));
                if (fullpath)
            ...
            g_list_free(folder_pairs);
        } else {
            GtkWidget *up_lab = gtk_label_new("  \U0001f4c1 ..");
            gtk_label_set_xalign(GTK_LABEL(up_lab), 0.0);
            GtkWidget *up_row = gtk_list_box_row_new();
            g_object_set_data(G_OBJECT(up_row), "song-idx", GINT_TO_POINTER(-1));
            gtk_container_add(GTK_CONTAINER(up_row), up_lab);
            gtk_list_box_insert(list, up_row, -1);
            gtk_widget_show_all(up_row);

            GDir *dir = g_dir_open(player->folder_current_path, 0, NULL);
            if (dir) {
                const gchar *name;
                while ((name = g_dir_read_name(dir)) != NULL) {
                    gchar *fullpath = g_build_filename(
                        player->folder_current_path, name, NULL);
                    if (g_file_test(fullpath, G_FILE_TEST_IS_DIR))
                        dirs = g_list_append(dirs, g_strdup(name));
                    g_free(fullpath);
                }
                g_dir_close(dir);
            }
            dirs = g_list_sort(dirs, (GCompareFunc)g_utf8_collate);
            ...
            for (GList *l = dirs; l; l = l->next) {
                ...
                gchar *fullpath = g_build_filename(
                    player->folder_current_path, name, NULL);
                GtkWidget *lab = gtk_label_new(NULL);
                gchar *markup = g_strdup_printf("  \U0001f4c1 %s", name);
                gtk_label_set_markup(GTK_LABEL(lab), markup);
                g_free(markup);
                gtk_label_set_xalign(GTK_LABEL(lab), 0.0);
                GtkWidget *row = gtk_list_box_row_new();
            ...
            for (GList *l = player->songs; l; l = l->next) {
                ...
                gchar *sdir = g_path_get_dirname(song->filepath);
                gboolean match = g_strcmp0(sdir, player->folder_current_path) == 0;
                g_free(sdir);
                if (!match) continue;
                if (!song_matches_search(song, search)) continue;
                song_items = g_list_append(song_items, song);
            }
            song_items = g_list_sort(song_items, (GCompareFunc)song_sort_by_title);

            for (GList *l = song_items; l; l = l->next) {
                Song *song = (Song *)l->data;
                gint idx = g_list_index(player->songs, song);
                if (idx < 0) continue;
                gchar *dn;
                if (song->metadata_loaded && strlen(song->artist) > 0)
                    dn = g_strdup_printf("      %s  -  %s",
                        song->artist, song->title);
                else {
                    gchar *base = g_path_get_basename(song->filepath);
                    dn = g_strdup_printf("      %s", base);
                    g_free(base);
                }
                GtkWidget *lab = gtk_label_new(dn);
                gtk_label_set_xalign(GTK_LABEL(lab), 0.0);
                GtkWidget *row = gtk_list_box_row_new();
                g_object_set_data(G_OBJECT(row), "song-idx",
        ...
        g_list_free(dirs);
        g_list_free(song_items);
    } else {
        gint idx = 0;
        for (GList *l = player->songs; l; l = l->next, idx++) {
            Song *song = (Song *)l->data;
            if (!song_matches_search(song, search)) continue;
            gchar *base = g_path_get_basename(song->filepath);
            gchar *display;
...
static void on_open_folder(GtkButton *btn, Player *player) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        player->songs = g_list_sort(player->songs, (GCompareFunc)song_sort_by_title);
...
static void player_go_up(Player *player) {
    if (!player->folder_current_path) return;
    gchar *parent = g_path_get_dirname(player->folder_current_path);
    gboolean is_tracked = FALSE;
    for (GList *l = player->folders; l; l = l->next) {
        if (g_strcmp0((gchar *)l->data, player->folder_current_path) == 0) {
            is_tracked = TRUE;
            break;
        }
    }
    g_free(player->folder_current_path);
    if (is_tracked || g_strcmp0(parent, ".") == 0) {
        player->folder_current_path = NULL;
    } else {
        player->folder_current_path = g_strdup(parent);
    }
    g_free(parent);
    player_refresh_playlist(player);
}

static void player_enter_folder(Player *player, const gchar *path) {
    g_free(player->folder_current_path);
    player->folder_current_path = g_strdup(path);
    player_refresh_playlist(player);
}

static void on_row_activated(GtkListBox *box, GtkListBoxRow *row, Player *player) {
    gint val = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(row), "song-idx"));
    if (player->tree_mode) {
        if (val == -1) {
            player_go_up(player);
            return;
        ...
        if (val == -2) {
            const gchar *path = g_object_get_data(G_OBJECT(row), "folder-path");
            if (path) player_enter_folder(player, path);
            return;
        }
    }
    if (val <= 0) return;
...
static void on_shuffle_toggled(GtkToggleButton *btn, Player *player) {
    player->shuffle = gtk_toggle_button_get_active(btn);
}

static void on_search_changed(GtkSearchEntry *entry, Player *player) {
    (void)entry;
    player_refresh_playlist(player);
}

static void on_tree_toggled(GtkToggleButton *btn, Player *player) {
    (void)btn;
    player->tree_mode = gtk_toggle_button_get_active(btn);
    if (player->tree_mode) {
        g_free(player->folder_current_path);
        player->folder_current_path = NULL;
    }
...
static gboolean on_key_press(GtkWidget *widget, GdkEventKey *event,
    ...
    if (event->keyval == GDK_KEY_space) {
        GtkWidget *focused = gtk_window_get_focus(GTK_WINDOW(player->window));
...
static void build_ui(Player *player) {
    ...
    g_signal_connect(player->window, "key-press-event", G_CALLBACK(on_key_press), player);
    ...
    player->shuffle_btn = gtk_toggle_button_new_with_label("\U0001f500");
    gtk_widget_set_name(player->shuffle_btn, "control-btn");
    g_signal_connect(player->shuffle_btn, "toggled",
        G_CALLBACK(on_shuffle_toggled), player);
    ...
    gtk_box_pack_end(GTK_BOX(controls), player->shuffle_btn, FALSE, FALSE, 0);
    ...
    player->search_entry = gtk_search_entry_new();
    gtk_widget_set_hexpand(player->search_entry, TRUE);
    g_signal_connect(player->search_entry, "changed",
        G_CALLBACK(on_search_changed), player);

    player->tree_toggle_btn = gtk_toggle_button_new_with_label("\U0001f4c2");
    gtk_widget_set_name(player->tree_toggle_btn, "menu-btn");
    g_signal_connect(player->tree_toggle_btn, "toggled",
        G_CALLBACK(on_tree_toggled), player);

    gtk_box_pack_start(GTK_BOX(toolbar), player->search_entry, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(toolbar), player->tree_toggle_btn, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(right_panel), toolbar, FALSE, FALSE, 2);
    ...
    GtkCssProvider *common_css = gtk_css_provider_new();
    gtk_css_provider_load_from_data(common_css,
        "label#folder-header { color: #8888cc; font-size: 11px; padding: 4px 0px; }\n"
        "button:checked { background: #3a3a6e; border: 1px solid #6a90e0; }\n"
        "button#control-btn:checked { background: #3a3a6e; border: 1px solid #6a90e0; }\n"
        "entry { background: #1a1a3e; color: #e0e0e0; border: 1px solid #2a2a5e; border-radius: 4px; }\n",
        -1, NULL);
    gtk_style_context_add_provider_for_screen(gdk_screen_get_default(),
        GTK_STYLE_PROVIDER(common_css),
        GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
